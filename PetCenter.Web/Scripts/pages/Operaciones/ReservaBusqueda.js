...
function fnc_Grid() {
    ...
    fncUtil_Grid("#jqGrid_lista_A", fnc_Listar, "CodigoReserva", colNames, colModel, "#jqGrid_pager_A", "FechaInicio", "asc", 
        function (id) {
            rowData = $("#jqGrid_lista_A").jqGrid('getRowData', id);

            $("#btnEliminar").prop("disabled", true);
            if (rowData.EstadoReserva == 1) $("#btnEliminar").prop("disabled", false);
...
function fnc_Delete() {
    ...
    objMessage.message = "¿Desea anular reservación?";
    objMessage.Question = true;
    objMessage.OnAccept = function () {
        fncUtil_Loading("show");

        var oItem = {};
        oItem.CodigoReserva = rowData.CodigoReserva;

        var oAjax = new fAjax();
        oAjax.Url = "Reserva/Anular";
        oAjax.Parameters = JSON.stringify(oItem);
        oAjax.OnDone = function (result) {
            fncUtil_Loading("hide");
            ...
            if (result == true) {
                objMessage = new fModalMessage();
                objMessage.message = "La reserva fue anulada con éxito";
                objMessage.Ok = true;
